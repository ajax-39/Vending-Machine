...
// Converts user input into an integer after validating it as a numeric string.
...
// Converts user input into a float after validating it includes only digits and at most one decimal point.
float inputCheckerFloat(string &s)
{
    ...
    do
    {
        ...
        // Validate that characters are digits or a single decimal point
        for (size_t i = 0; i < s.length(); i++)
        {
            if (!(s[i] >= '0' && s[i] <= '9') && s[i] != '.')
            {
...
class Administration : public Product
{
private:
    int pin;
...
public:
    ...
    void displayProductQuantities(const vector<Product> &products)
    {
    ...
    void editInventory(vector<Product> &beverages, vector<Product> &snacks, vector<Product> &candies, vector<Product> &biscuits)
    {
        ...
        displayProductQuantities(beverages);
        displayProductQuantities(snacks);
        displayProductQuantities(candies);
        displayProductQuantities(biscuits);
...
int main()
{
    ...
    cout << endl
         << endl;
    cout << setw(60) << "Welcome to Vending Machine!" << endl;
